...
let restartManager: ReturnType<typeof loadRestartManager> | null = null;
...
function parseIntervalMilliseconds(value: string) {
  if (!/^\d+$/.test(value)) {
    return null;
  }
...
function getResourcePaths(targetPath: string) {
  ...
  const resourcePaths: string[] = [targetPath];
...
function getLockingProcessInfo(resourcePaths: string[]) {
  const processesById = new Map<number, LockingProcessInfo>();
...
function addLockingProcessInfo(processesById: Map<number, LockingProcessInfo>, processes: LockingProcessInfo[]) {
  for (const processInfo of processes) {
    processesById.set(processInfo.processId, processInfo);
  }
}

function getLockingProcessesForBatch(resourcePaths: string[]) {
  ...
  try {
    ...
    const lockingProcesses: LockingProcessInfo[] = [];
...
function readProcessInfo(buffer: Buffer, offset: number): LockingProcessInfo {
  return {
...
function readNullTerminatedUtf16(buffer: Buffer, offset: number, maxCharacters: number) {
...
function stopLockingProcesses(lockingProcesses: LockingProcessInfo[]) {
  let hadError = false;
...
function throwIfFailed(result: number, operation: string) {
  if (result !== ERROR_SUCCESS) {
